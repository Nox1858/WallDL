...
def setWallpaper_Android(wallpaper_path, img_id, ctx: AppContext, raw = False):
    subprocess.Popen(f'termux-wallpaper -f {wallpaper_path}', shell=True)
    if(not raw):
        add_occurance(img_id, ctx)


def setWallpaper_MacOS(wallpaper_path, img_id, ctx: AppContext, raw = False):
...
def setWallpaper_Plasma(wallpaper_path, img_id, ctx: AppContext, raw = False):
    subprocess.run(
...
def setRawWallpaper(wallpaper_path, ctx) -> None:
    DESKTOP_MANAGER = ctx.desktop_manager
    match DESKTOP_MANAGER:
        case "Plasma":
            setWallpaper_Plasma(wallpaper_path, 0, ctx, True)
        case "MacOS":
            setWallpaper_MacOS(wallpaper_path, 0, ctx, True)
        case "Android":
            setWallpaper_Android(wallpaper_path, 0, ctx, True)
